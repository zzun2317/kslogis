...
export default function DriverAdminPage() {
  ...
  const [dateSearchType, setDateSearchType] = useState<'DEV' | 'ORD'>('DEV'); // 배송일자 검색 기준 선택 (DEV: 배송일자, ORD: 수주일자)
  ...
  useEffect(() => {
    ...
    const fetchCommonData = async () => {
      // 기사 목록 로드
      const { data: drv } = await supabase.from('ks_driver').select('driver_id, driver_name, driver_email, driver_center, driver_uuid, driver_carno').in('driver_center', userCenterList).order('driver_name');
      if (drv) setDrivers(drv);
      // 물류사 코드(004) 로드
      const { data: dc } = await supabase.from('ks_common').select('comm_ccode, comm_text1').eq('comm_mcode', '004').in('comm_ccode', userCenterList).order('comm_ccode');
      if (dc) setDevcenterList(dc);
      // 배송 상태 코드(002) 로드
      const { data: st } = await supabase.from('ks_common').select('comm_ccode, comm_text1, comm_hex').eq('comm_mcode', '002').in('comm_ccode', ['002001', '002006', '002007']).order('comm_ccode');
  ...
  const fetchDeliveryData = useCallback(async () => {
    ...
    const rpcParams = {
      ...
      p_dev_date: searchDate,      
      p_date_type: dateSearchType,
      p_center_code: userCenterList.join(','),
      p_status_codes: searchStatus === '전체' ? '%' : searchStatus,
    ...
    try {
      const { data, error } = await supabase.rpc('get_driver_admin_delivery_list', rpcParams);
  ...
  }, [dateSearchType, searchDate, reqDate, gubun, searchDevcenter, searchStatus, custName, hp, address, driver, user, userCenterList]);
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-3">
            ...
            <div className="flex items-center gap-1.5 sm:border-l sm:pl-4 border-slate-200">
              ...
              >
                ...
                {(!isLocalManager || userCenterList.length > 1)}
            ...
            <div className="flex items-center gap-1 relative">
              ...
              {/* 사용자에게 보여지는 디자인용 박스 */}
              <div 
                className={`${inputStyle} w-[140px] px-3 flex items-center gap-2.5 shadow-sm bg-white transition-all hover:border-blue-600 group`}
              >
                <span className="text-[12px] font-black text-slate-800 tracking-tight">
                  {searchDate.replace(/-/g, '.')}
                </span>
                
                {/* 🚀 화살표 대신 2D 달력 SVG 아이콘으로 교체 */}
                <svg 
            ...
            <div className="flex flex-wrap items-center gap-1.5 sm:border-l sm:pl-4 border-slate-200">
              ...
              <select 
                ...
                disabled={!canEdit && !isDriverAdmin}
              ...
              <button 
                ...
                disabled={loading || selectedRows.length === 0 || !selectedTargetDriver || (!canEdit && !isDriverAdmin)} 
